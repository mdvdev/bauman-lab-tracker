...
// Forked from Microsoft.AspNetCore.Identity.IdentityApiEndpointRouteBuilderExtensions
// Source: https://github.com/dotnet/aspnetcore/blob/main/src/Identity/Core/src/IdentityApiEndpointRouteBuilderExtensions.cs
// Reason: Customized /register endpoint to accept custom DTO (RegisterDto) with extra fields.
// Note: Check for updates in ASP.NET Core Identity in case of future changes
...
public static class IdentityApiEndpointRouteBuilderExtensions
{
    ...
    /// </summary>
    /// <typeparam name="TUser">The type describing the user. This should match the generic parameter in <see cref="endpoints"/>.</typeparam>
    /// <param name="endpoints">
    /// The <see cref="EndpointRouteBuilderExtensions.MapGroup(Microsoft.AspNetCore.Routing.IEndpointRouteBuilder,string)"/> to add the identity endpoints to.
    /// Call <see cref="EndpointRouteBuilderExtensions"/> to add a prefix to all the endpoints.
    /// </param>
    /// <returns>An <see cref="UserManager{TUser}"/> to further customize the added endpoints.</returns>
            ...
            ([FromBody] RegisterDto registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            if (!userManager.SupportsUserEmail)
            {
                throw new NotSupportedException(
                    $"{nameof(MapCustomizedIdentityApi)} requires a user store with email support.");
            }
            ...
            var user = new TUser
            {
                ...
                FirstName = new Name(registration.FirstName),
                LastName = new Name(registration.LastName),
                Patronymic = new Name(registration.Patronymic),
            };
